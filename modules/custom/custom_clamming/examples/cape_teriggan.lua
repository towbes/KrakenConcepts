...
clamming.zone[xi.zone.CAPE_TERIGGAN] =
{
    ...
    drops =
    {
        ...
        { rate.RARE,        weight.VERY_HEAVY, xi.item.RUSTY_GREATSWORD,        'a rusty greatsword'        }, --  5% 20pz
        { rate.RARE,        weight.VERY_LIGHT, xi.item.BROKEN_LINKPEARL,        'a broken linkpearl'        }, --  5% 3pz
        { rate.RARE,        weight.MODERATE,   xi.item.CORAL_FRAGMENT,          'a coral fragment'          }, --  5% 7pz
        { rate.RARE,        weight.HEAVY,      xi.item.HIGH_QUALITY_CRAB_SHELL, 'a high-quality crab shell' }, --  5% 11pz
        { rate.RARE,        weight.LIGHT,      xi.item.PIECE_OF_OXBLOOD,        'a piece of oxblood'        }, --  5% 6pz
        { rate.RARE,        weight.VERY_LIGHT, xi.item.PIECE_OF_ANGEL_SKIN,     'a piece of angel skin'     }, --  5% 3pz
    },
...
m:addOverride('xi.zones.Cape_Teriggan.Zone.onInitialize', function(zone)
    super(zone)
...
m:addOverride('xi.zones.Cape_Teriggan.Zone.onZoneIn', function(player, zonePrev)
    super(player, zonePrev)
    clamming.onZoneIn(player)
end)

m:addOverride('xi.zones.Cape_Teriggan.Zone.onZoneOut', function(player)
    super(player)

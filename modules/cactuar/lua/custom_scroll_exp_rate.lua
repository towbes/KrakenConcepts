...
then
    m:addOverride('xi.globals.item.copy_of_ginuvas_battle_theory.onItemUse', function(target)
        target:addExp(xi.settings.main.EXP_RATE * math.random(50, 200) * xi.settings.main.SCROLL_EXP_RATE)
    end)

    m:addOverride('xi.globals.item.copy_of_schultz_stratagems.onItemUse', function(target)
      target:addExp(xi.settings.main.EXP_RATE * math.random(150, 500) * xi.settings.main.SCROLL_EXP_RATE)
    end)

    m:addOverride('xi.globals.item.page_from_balrahns_reflections.onItemUse', function(target)
      target:addExp(xi.settings.main.EXP_RATE * math.random(200, 500) * xi.settings.main.SCROLL_EXP_RATE)
    end)

    m:addOverride('xi.globals.item.page_from_miratetes_memoirs.onItemUse', function(target)
      target:addExp(xi.settings.main.EXP_RATE * math.random(750, 1500) * xi.settings.main.SCROLL_EXP_RATE)
    end)

    m:addOverride('xi.globals.item.page_from_the_dragon_chronicles.onItemUse', function(target)

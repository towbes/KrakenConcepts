...
then
    m:addOverride('xi.items.copy_of_ginuvas_battle_theory.onItemUse', function(target)
        target:addExp(xi.settings.main.EXP_RATE * math.random(50, 200) * xi.settings.main.SCROLL_EXP_RATE)
    end)

    m:addOverride('xi.items.copy_of_schultz_stratagems.onItemUse', function(target)
      target:addExp(xi.settings.main.EXP_RATE * math.random(150, 500) * xi.settings.main.SCROLL_EXP_RATE)
    end)

    m:addOverride('xi.items.page_from_balrahns_reflections.onItemUse', function(target)

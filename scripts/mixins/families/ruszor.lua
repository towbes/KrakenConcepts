...
local wsAftermath =
{
    [2438] = { mod = xi.mod.UDMGPHYS,     value = -100, duration = 30,  animsub = 1, },    -- Frozen Mist aftermath
    [2439] = { mod = xi.mod.WATER_ABSORB, value = 100,  duration = 60, animsub = 2, },     -- Aqua Wave aftermath
}
...
g_mixins.families.ruszor = function(mob)
    ...
    mob:addListener('WEAPONSKILL_USE', 'RUSZOR_WSUSE', function(mob, _, wsid)
        ...
        if wsid == 2438 or wsid == 2439 then
            ...
            mob:setAnimationSub(aftermath.animsub)
        end
    end)
